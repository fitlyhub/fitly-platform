...
create table if not exists sys_position_role_scope
(
    sys_position_role_scope_id uuid                    not null
        primary key,
    sys_position_id            uuid                    not null
        references sys_position,
    sys_role_id                uuid                    not null
    ...
    sys_org_id                 uuid
        references sys_org,
    segment_scope              varchar(100)            not null,
    sys_segment_id             uuid
        references sys_segment,
    is_active                  boolean   default true  not null,
    created_at                 timestamp default now() not null,
